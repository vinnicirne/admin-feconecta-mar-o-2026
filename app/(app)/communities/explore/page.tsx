  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "20px 16px 120px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h1 style={{ fontSize: "1.8rem", fontWeight: 900, margin: 0 }}>Comunidades</h1>
        ...
        <div className="grid" style={{ gap: 12 }}>
          {filtered.map(c => (
             <Link key={c.id} href={`/communities/${c.id}`} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px", borderRadius: 16, background: "white", border: "1px solid var(--line)", textDecoration: "none", color: "inherit", boxShadow: "0 2px 10px rgba(0,0,0,0.02)" }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(217, 119, 6, 0.1)", display: "grid", placeItems: "center", color: "var(--accent)" }}>
                <Globe size={20} />
              </div>
              <div>
                <strong style={{ display: "block", fontSize: 15 }}>{c.name}</strong>
                <span style={{ fontSize: 12, color: "var(--muted)" }}>{c.category || "Comunidade"}</span>
              </div>
            </Link>
          ))}
          {filtered.length === 0 && <p className="muted" style={{ textAlign: "center", padding: 40 }}>Nenhuma comunidade encontrada.</p>}
        </div>
      )}
    </div>
  );
